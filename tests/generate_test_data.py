...
Automated test data generator, for synthetic datasets of any scaling limit
Outputs in-memory string variables
"""
import random

def generate_synthetic_xml_and_csv(num_rules: int, num_objects: int, flaw_ratio: float = 0.1) \
        -> tuple[str, str, dict]:
    """
    ...
    :param flaw_ratio: probability that next generated rule/object will be intentionally flawed
    :return: configuration tree, containing rules and objects, along with the dictionary, to be used for testing
    """
    ground_truth = {
        "contradictions": 0,
        "decommissioned": 0,
    ...
    for i in range(num_objects):
        ...
        is_flawed = random.random() < flaw_ratio

        if is_flawed:
            anomaly_type = random.choice(["decommissioned", "mismatch"])
